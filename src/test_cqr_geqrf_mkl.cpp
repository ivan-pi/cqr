...
int suite1(int nm, int m, int n, double cond, Structure structure = DENSE)
{
    ...
    cqr_mkl_dgeqrf_compact(MKL_COL_MAJOR, m, n, ap, m, taup, &wq, (MKL_INT)wq, &info, fmt, nm);
    ...
    for (int v = 0; v < nm; ++v) {
        ...
        cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, k, n, m,
                    1.0, Q.data(), m, Av, m, 0.0, QtA.data(), k);
        double resid = 0;
        for (int j = 0; j < n; ++j)
            for (int i = 0; i < k; ++i) {
        ...
        cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, k, k, m,
                    1.0, Q.data(), m, Q.data(), m, 0.0, QtQ.data(), k);
        for (int i = 0; i < k; ++i) QtQ[i + (size_t)i * k] -= 1.0;
        ...
        double el = std::max(maxdiff(Hv, Href.data(), sA), maxdiff(tv, tauref.data(), sT));
...
int suite2(MKL_LAYOUT layout, int nm, int m, int n)
{
    ...
    std::vector<double> A(nm * sA);
    for (int v = 0; v < nm; ++v) gen_matrix(A.data() + v * sA, m, n, 0.0);
    ...
    MKL_INT info = 0;
    double wq;
    cqr_mkl_dgeqrf_compact(layout, m, n, ap1.get(), ldc, tp1.get(), &wq, -1, &info, fmt, nm);
    cqr_mkl_dgeqrf_compact(layout, m, n, ap1.get(), ldc, tp1.get(), &wq, (MKL_INT)wq, &info, fmt, nm);
    std::vector<double> work((size_t)std::max<MKL_INT>((MKL_INT)wq, 1));
    mkl_dgeqrf_compact(layout, m, n, ap2.get(), ldc, tp2.get(), work.data(), (MKL_INT)work.size(), &info, fmt, nm);
    ...
    std::printf("  [suite2] %s V=%-2d nm=%-2d m=%-3d n=%-3d | max|ap-mkl| %.2e max|tau-mkl| %.2e (tol %.0e) %s\n",
...
int suite3(int nm, int n, int nrhs)
{
    ...
    cqr_mkl_dormqr_compact(MKL_COL_MAJOR, 'L', 'T', m, nrhs, k, ap, m, taup, cp, m, &wq, -1, &info, fmt, nm);
    cqr_mkl_dormqr_compact(MKL_COL_MAJOR, 'L', 'T', m, nrhs, k, ap, m, taup, cp, m, &wq, (MKL_INT)wq, &info, fmt, nm);
    ...
    fails += !ok;
    std::printf("  [suite3] V=%-2d nm=%-2d n=%-3d nrhs=%d | fwd %.2e res %.2e (rtol %.2e) %s\n",
