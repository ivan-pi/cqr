...
// factorization (dsytrfnp_compact / ssytrfnp_compact) and its solve companion
// (dsytrsnp_compact / ssytrsnp_compact), with no BLAS dependency. The reference
// is the unblocked unpivoted factorization implemented in scalar form -- the
// same algorithm the vectorized kernel executes V lanes at a time, so a correct
...
//   4. end-to-end solve: factor + ?sytrsnp_compact recovers a known X
// plus:
...
//   - LAPACK-style argument validation of both C APIs.
//
// Assisted-by: Claude
...
#include "cqr_compact.h"         // dsytrfnp_compact / dsytrsnp_compact (+ s)
#include "test_compact_util.hpp" // frand, gen_sym_ldlt, MatrixBatch, pack/unpack
...
static int sytrfnp_c(char lay, char up, int n, double *a, int ld, int V, int nm)
{
    return dsytrfnp_compact(lay, up, n, a, ld, V, nm);
}
static int sytrfnp_c(char lay, char up, int n, float *a, int ld, int V, int nm)
{
    return ssytrfnp_compact(lay, up, n, a, ld, V, nm);
}
static int sytrsnp_c(char lay, char up, int n, int nrhs, const double *a, int lda,
                     double *b, int ldb, int V, int nm)
{
    return dsytrsnp_compact(lay, up, n, nrhs, a, lda, b, ldb, V, nm);
}
static int sytrsnp_c(char lay, char up, int n, int nrhs, const float *a, int lda,
                     float *b, int ldb, int V, int nm)
{
    return ssytrsnp_compact(lay, up, n, nrhs, a, lda, b, ldb, V, nm);
}

// Reconstruct A(i,j) from the packed factor of matrix idx: sum over
// l <= min(i,j) of F(i,l) * D(l) * F(j,l) with F unit on the diagonal (lower;
// the upper convention reads F(l,i) * D(l) * F(l,j)).
template <class T>
static double reconstruct(const MatrixBatch<T> &F, int idx, int i, int j, int n,
                          bool upper)
{
    double s = 0;
    for (int l = 0; l <= std::min(i, j); ++l) {
        double fi = upper ? (double)F(idx, l, i) : (double)F(idx, i, l);
        double fj = upper ? (double)F(idx, l, j) : (double)F(idx, j, l);
        if (l == i) fi = 1.0;
        if (l == j) fj = 1.0;
        s += fi * (double)F(idx, l, l) * fj;
    }
    return s;
}
...
template <class T, int V> static int run_case(int nm, int n, char uplo, char layout)
{
    ...
    int info = sytrfnp_c(layout, uplo, n, ap.data(), n, V, nm);
    ...
    for (int idx = 0; idx < nm; ++idx) {
        ...
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                e_rec = std::max(e_rec, std::abs(reconstruct(Aout, idx, i, j, n, upper) -
                                                 (double)A(idx, i, j)));
    }
...
static int run_solve(int nm, int n, int nrhs, char uplo, char layout)
{
    ...
    // known X (constant per column), B = A X computed densely
    ...
    pack_compact(A, ap.data(), n, V, rowmajor);
    pack_compact(B, bp.data(), rowmajor ? nrhs : n, V, rowmajor);

    int info_f = sytrfnp_c(layout, uplo, n, ap.data(), n, V, nm);
    int info_s = sytrsnp_c(layout, uplo, n, nrhs, ap.data(), n, bp.data(),
                           rowmajor ? nrhs : n, V, nm);

    MatrixBatch<T> Xhat(nm, n, nrhs);
    unpack_compact(Xhat, bp.data(), rowmajor ? nrhs : n, V, rowmajor);
    ...
    bool ok =
        (e_fwd <= rtol_fwd) && (e_res <= rtol_res) && (info_f == 0) && (info_s == 0);
    std::printf("T=%-6s V=%-2d uplo=%c lay=%c nm=%-2d n=%-3d nrhs=%d solve | fwd:%.1e "
                "(%.1e) res:%.1e (%.1e) info=%d/%d %s\n",
                sizeof(T) == 8 ? "double" : "float", V, uplo, layout, nm, n, nrhs, e_fwd,
                rtol_fwd, e_res, rtol_res, info_f, info_s, ok ? "OK" : "FAIL");
    return !ok;
}
...
template <class T, int V> static int run_zerodiag(char uplo, char layout)
{
    ...
    std::printf("T=%-6s V=%-2d uplo=%c lay=%c zero-diagonal A(1,1)=0 | err:%.1e "
                "finite:%s %s\n",
                sizeof(T) == 8 ? "double" : "float", V, uplo, layout, e,
                finite ? "yes" : "NO", ok ? "OK" : "FAIL");
    return !ok;
}
...
template <class T, int V> static int run_zeropivot(int n, char uplo, char layout)
{
    ...
    std::printf("T=%-6s V=%-2d uplo=%c lay=%c n=%-3d zero-pivot lane | siblings:%.1e %s "
                "poison:%s info=%d %s\n",
                sizeof(T) == 8 ? "double" : "float", V, uplo, layout, n, e_sib,
                ok_sib ? "OK" : "FAIL", ok_bad ? "OK" : "FAIL", info,
                ok_info ? "OK" : "FAIL");
...
int main()
{
    ...
    // (factor + sytrsnp), over uplo/layout/precision and padded groups.
    for (char u : uplos)
        for (char l : lays) {
