...
right-hand sides, time steps, or Newton iterations --- to amortize it. A third
strategy keeps the per-matrix layout but draws its parallelism from the execution
policy rather than the data layout, with the operation's properties fixed at
compile time --- the route Kokkos Kernels takes (Section~\ref{sec:related}).
...
Storage             & interleaved & interleaved & per-matrix (Views) & interleaved \\
Cross-matrix work   & SIMD lanes & SIMD lanes & execution policy (Serial / Team / TeamVector) & SIMD lanes \\
...
The interleaved libraries put one \emph{matrix} in each SIMD lane, so a fixed
group of $V$ matrices advances in lockstep. Kokkos instead exposes three interface
levels that choose how many workers cooperate on a \emph{single} matrix, leaving
the batch to the surrounding \code{parallel\_for}: \code{Serial} runs a whole
matrix on one execution unit (one CPU thread, or one GPU thread), its inner loops
left to the compiler; \code{Team} shares one matrix across a team of threads
(\code{TeamThreadRange}); and \code{TeamVector} adds the innermost vector level
(\code{ThreadVectorRange}) so a team's threads \emph{and} their vector lanes
jointly work one matrix. \code{Serial} over a plain range --- one small matrix per
thread --- is the natural fit for the many-tiny-matrices regime targeted here;
\code{Team} and \code{TeamVector} pay off as matrices grow or thin out. It is a
different lever from interleaving, where a lane always holds a \emph{distinct}
matrix rather than a slice of a shared one.
...
Kernels}~\cite{kokkos-kernels} takes the other route: each matrix keeps its
ordinary layout (no interleaving), parallelism comes from the execution policy ---
a \code{Serial}, \code{Team}, or \code{TeamVector} variant choosing whether one
thread, a team, or a team-plus-vector-lane handles a matrix --- and the operation
properties (transpose, upper/lower, side, and the blocking \code{Algo}) are
\emph{compile-time template arguments}, so each kernel is a header-only
\code{invoke()} that inlines into the user's \code{parallel\_for} and, through
Kokkos, runs on GPUs as well as CPUs. The trade is instructive: interleaving
guarantees cross-matrix SIMD on the CPU but needs a pack/unpack and a
CPU-specific layout, whereas the Kokkos style fuses into user code and ports to
accelerators but leans on the execution policy and the compiler for
vectorization. \draftnote{Kokkos Kernels also ships a SIMD vector value type that
can interleave matrices on-chip, much like the compact packs; confirm its current
status in \code{develop} and whether to mention it.} \cqr{} sits in the
interleaved camp, as does the \code{batmat} project~\cite{batmat} (aimed at convex
optimization); its contribution relative to all of these is narrow and specific:
an \emph{open} drop-in for the MKL compact QR, portable across ISAs from one
source, that supplies the apply-$Q$ step MKL omits. \draftnote{if targeting a venue, add the closest
