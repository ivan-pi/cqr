...
factorization. \draftnote{record the LIKWID kernel used and the measured peak for
each machine, at the benchmark's core count.}
...
things to read off them. First, how far every driver sits below the ceiling:
these are small, unblocked, latency- and memory-bound factorizations, so a large
gap to the FMA peak is expected and is shared by all three --- the interesting
question is relative standing, not absolute peak fraction. Second, the compact
paths' advantage over the per-matrix loop at equal $n$, now in absolute units.
Third, the gentle staircase on the \cqr{} curve, dipping at orders that are not
multiples of the interleave width --- the remainder effect of
Section~\ref{sec:cqr}, and the reason the size list includes the odd stencil
sizes. The rate also climbs and saturates as $n$ grows, as the vector units fill.
...
\emph{Placeholder data; placeholder roofline.}}
