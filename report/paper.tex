...
compact LAPACK is \emph{incomplete} for the QR pipeline: it provides QR,
Cholesky, and unpivoted-LU factorizations and a triangular solve, yet neither
\code{?ormqr} nor \code{?orgqr} --- no supported way to apply or form the
orthogonal factor $Q$, the step that turns a QR factorization into a
least-squares solve.
...
right-hand sides, time steps, or Newton iterations --- to amortize it. Parallelism
can also come from an execution policy --- one matrix per thread or team --- rather
than the data layout, and the cross-matrix SIMD of the interleaved approach can be
had through a vector \emph{element type} instead of a dedicated storage format;
Kokkos Kernels offers both (Section~\ref{sec:related}).
...
\begin{table}[t]
\centering
\caption{Coverage of the compact / interleaved batched APIs. Routine stems are
shown without the precision letter and the vendor's suffix (MKL
\code{mkl\_?X\_compact}; ArmPL \code{armpl\_?X\_interleave\_batch}). \cmark{}~=
provided, --- = absent. Variant tags: \textsc{np} no pivoting, \textsc{tp}
threshold pivoting, \textsc{rr} rank-revealing. Packing is MKL
\code{?gepack}/\code{?geunpack} (with \code{get\_format} / \code{get\_size})
versus ArmPL \code{?ge\_interleave}/\code{?ge\_deinterleave} (strides passed
explicitly).}
\label{tab:apis}
\footnotesize
\begin{tabular}{@{}llll@{}}
...
\midrule
\multicolumn{4}{@{}l}{\itshape BLAS}\\
\quad \code{dot}, \code{scal}, \code{ger}, \code{gemv} & --- & \cmark & --- \\
\quad \code{gemm}                & \cmark & \cmark & --- \\
\quad \code{trmm}, \code{trsv}   & ---    & \cmark & --- \\
\quad \code{trsm}                & \cmark & \cmark & --- \\
\multicolumn{4}{@{}l}{\itshape LAPACK}\\
\quad Cholesky \code{potrf}      & \cmark & \cmark & design \\
\quad LU \code{getrf}            & \cmark\,\textsc{np} & \cmark\,\textsc{tp} & --- \\
\quad LU inverse \code{getri}    & \cmark\,\textsc{np} & --- & --- \\
\quad QR \code{geqrf}            & \cmark & \cmark\,\textsc{rr} & \cmark \\
\quad form $Q$ \code{orgqr}      & ---    & \cmark & --- \\
\quad apply $Q$ \code{ormqr}     & \textbf{---} & \cmark & \cmark \\
\bottomrule
\end{tabular}
\end{table}
...
from \code{mkl\_get\_format\_compact()}, with \code{?gepack\_compact} /
\code{?geunpack\_compact} moving data in and out~\cite{mkl-compact}. Its kernels
(Table~\ref{tab:apis}) are compact GEMM and TRSM, Cholesky, an \emph{unpivoted}
LU and its inverse (\code{?getrfnp}, \code{?getrinp}), and QR (\code{?geqrf}). The
LU is unpivoted by name and by design: partial pivoting's per-column search and
row interchange do not vectorize cleanly across a pack, so MKL forgoes them. The
conspicuous hole is in QR --- MKL ships neither \code{?ormqr} nor \code{?orgqr},
so a caller can obtain the reflectors $(H,\tau)$ and triangular-solve, but has no
supported way to \emph{apply} $Q^{\mathsf T}$ to a right-hand side, the step that
turns the factorization into a least-squares solve. MKL's compact routines also
skip argument checking ``for performance reasons'' and leave \code{info}
reserved~\cite{mkl-compact}, with documented limits near underflow/overflow and
no pivoting~\cite{mkl-compact-limits}.
...
outer, and batch strides describing the interleave directly, and
\code{?ge\_interleave} / \code{?ge\_deinterleave} pack and unpack~\cite{armpl-interleave}.
Its coverage is the broadest of the three (Table~\ref{tab:apis}): the BLAS-1/2/3
building blocks (\code{dot}, \code{scal}, \code{ger}, \code{gemv}, \code{gemm},
\code{trmm}, \code{trsv}, \code{trsm}) and a \emph{complete} QR pipeline ---
rank-revealing factorization (\code{?geqrfrr}), \code{?orgqr} to form $Q$, and
\code{?ormqr} to apply it --- besides Cholesky and LU. Alone among the three it
pivots at all in the compact setting: its LU uses \emph{threshold} pivoting
(\code{?getrftp}). In the usual sense of the term, the in-place (diagonal) pivot
is accepted while it exceeds a fixed fraction of the largest candidate in its
column, and a row interchange is taken only when it does not --- so most lanes
stay on the cheap no-swap path and only the ill-conditioned ones diverge, trading
a little stability for a batch pattern that stays mostly uniform.
\draftnote{confirm ArmPL's exact \code{getrftp} pivoting semantics against their
...
for \code{cqr\_mkl\_?geqrf\_compact} unchanged, feed the result to MKL's
\code{?trsm\_compact}, and use \code{cqr\_mkl\_?ormqr\_compact} for the apply-$Q$
step MKL lacks; a second, MKL-free C API takes the width $V$ explicitly for use
without MKL. Its scope is deliberately narrow (Table~\ref{tab:apis}): the QR
factorization, the apply-$Q$ that completes the solve, and a designed-but-unbuilt
Cholesky. It provides no batched GEMM --- the one kernel that already vectorizes
well per matrix and that the vendors tune hard, so there is little to gain from
re-implementing it --- and no LU, since the pivoting question (forgo it like MKL,
or threshold it like ArmPL) is a study in itself. What \cqr{} adds over both
vendors is not breadth but openness and portability: one vector-typed source, no
ISA lock (below).

\begin{table}[t]
\centering
\caption{Design choices behind the four batched libraries --- the ``how'',
complementing the ``what'' of Table~\ref{tab:apis}. (Kokkos Kernels is not a
compact API, so it is absent from Table~\ref{tab:apis} but belongs here.)}
\label{tab:design}
\footnotesize
\begin{tabularx}{\linewidth}{@{}l XXXX@{}}
\toprule
Design choice & MKL Compact & ArmPL interleave-batch & Kokkos Kernels & \cqr{} \\
\midrule
...
\paragraph{Dispatch: a C ABI at runtime, or C++ templates at compile time.}
Intel and Arm ship \emph{raw C functions}. That buys ABI stability and
callability from C, C++, Fortran, and Python, at the price of resolving the
storage format and vector width \emph{at runtime}, inside a precompiled binary.
Kokkos makes the opposite choice: the properties and the algorithm are C++
template parameters resolved at \emph{compile time}, maximizing inlining and
fusion but restricting the caller to C++ and requiring a recompile per
configuration. Neither is strictly better --- the runtime indirection is a fixed
cost that matters most when the work per call is small, and whether a
compile-time-specialized library wins in that regime is exactly the open question.
\cqr{} takes a hybrid position (Table~\ref{tab:design}): it keeps the vendor's C
ABI --- so it stays a drop-in and remains callable from Fortran and Python --- but
resolves the pack format to a compile-time interleave width \emph{once per call}
and dispatches to a kernel fully specialized on $(T,V)$, so the inner sweep
carries no runtime branch. The runtime choice is paid once per batch, not once per
element.

\paragraph{Parallelism granularity.}
Two orthogonal levers set how a batch maps to the hardware, and it helps to keep
them apart. \emph{Across} matrices, a SIMD lane can carry a whole distinct matrix
so $V$ advance in lockstep --- what the compact storage format delivers directly,
and what Kokkos delivers through a SIMD value type
(Section~\ref{sec:related}); either way the cross-matrix SIMD is there. \emph{Within}
a matrix, the work can be done by one worker or shared among several --- the axis
Kokkos exposes as \code{Serial} (one thread per matrix), \code{Team} (a team of
threads per matrix), and \code{TeamVector} (team threads plus their vector lanes
per matrix). The compact libraries fix the first lever (one matrix per lane) and
leave the batch loop to the caller's threads; Kokkos exposes both, and --- being
built on Kokkos --- maps the second onto GPU threads, where CPU-style SIMD lanes
do not exist. For the many-tiny-matrices regime targeted here, one matrix per lane
and one matrix per thread are the natural CPU mappings; the \code{Team} levels
earn their keep as matrices grow or thin out.
...
\paragraph{Why explicit vector types, not \code{\#pragma omp simd}.}
The tempting alternative is to keep the plain scalar \code{geqr2} and annotate the
batch loop with \code{\#pragma omp simd simdlen(V)}, letting the compiler vectorize
across matrices. We tried it --- as a literal drop-in behind the same C entry
points --- and measured it against MKL on GCC, Clang, and icpx. The explicit vector
types were the robust choice: they beat \code{mkl\_?geqrf\_compact} by
$1.24$--$1.30\times$ (geometric mean) on \emph{every} compiler, with no flag
sensitivity. The omp-simd kernels never won --- their ceiling was MKL parity ---
and the clean outer-loop form (the single-matrix algorithm verbatim, pragma on the
batch loop) vectorized on \emph{only} icpx's dedicated OpenMP-SIMD backend
($1.13\times$); GCC's and Clang's auto-vectorizers drop the pragma on a loop that
wraps a nested body and fall back to scalar ($0.1$--$0.2\times$). An inner-loop
variant reaches parity portably ($0.97$--$1.00\times$) but only by splitting the
loops and adding per-column temporaries. The lesson: \code{\#pragma omp simd} on
the batch asserts iteration \emph{independence}, not the loop \emph{interchange}
that outer-loop batch vectorization needs, so whether it happens turns on the
lowering path, not the compiler brand. Explicit vector types make the SIMD
unconditional, which is why \cqr{} uses them. \draftnote{name the CPU and exact
compiler builds for these geomeans, and re-confirm against the omp-simd branch
before publication.}
...
(Figure~\ref{fig:gflops-x86}); the effect is most visible at the smallest orders and
...
argument checking on the MKL-style surface (like MKL), does not pivot (the row
interchanges of pivoting diverge across a pack), and does not implement
...
\paragraph{Machines.}
\textbf{x86 (provisional):} an AVX-512 Xeon (VNNI-capable) at 2.8\,GHz, 4 vCPUs
on a shared virtual machine; GCC~13.3 with \code{-O3 -march=native}; Intel MKL
(Ubuntu \code{libmkl}). \draftnote{shared, non-pinned VM --- provisional only;
replace with the empty pinned node's specs and re-run.}
\textbf{Arm:} \draftnote{AArch64 CPU, SVE vector length, cores, ArmPL version,
compiler and flags.}

\paragraph{Workload.}
Each data point factors a pool of $512$ random, well-conditioned square matrices
of a fixed order $n$, in double precision. The
...
pass. Timing uses Google Benchmark: a warm-up, then $10$ repetitions per point,
reported as the median. The two compact
...
\paragraph{Roofline.}
The GFLOP/s figures overlay the node's attainable double-precision compute
peak, measured with LIKWID~\cite{likwid2010} on the same node and core count as
the runs (the \code{likwid-bench} peak-flops kernels; the kernel's achieved rate
can also be read back with \code{likwid-perfctr -g FLOPS\_DP}). Reading the
achieved rate against that ceiling is a roofline view~\cite{roofline2009} of the
factorization. For the provisional x86 machine the ceiling is $316.9$\,GFLOP/s
(\code{likwid-bench -t peakflops\_avx512\_fma}, 4 threads, ${\sim}88\%$ of the
2.8\,GHz AVX-512 FMA theoretical peak). \draftnote{measure the Arm peak, and
re-measure x86 on the pinned node at the run's core count.}

\paragraph{Reproducibility.}
The x86 numbers come from the report's Google Benchmark collector
(\code{report/bench}, a separate CMake project); the ArmPL numbers require the
AArch64 harness described in Section~\ref{sec:future}. Scripts, data, and this document build with a single
...
\draftnote{The x86 figures show \textbf{real measurements}, but from a shared,
non-frequency-pinned virtual machine --- treat them as \emph{provisional} and
re-run on the target node for the paper. The Arm figures remain \textbf{synthetic
placeholders} pending hardware (Section~\ref{sec:future}). See
\code{report/data/README.md}.}}}\end{center}

\paragraph{Throughput on x86 (vs MKL Compact).}
Figure~\ref{fig:tput-x86} plots throughput (matrices/s) against order for \cqr{},
\code{mkl\_?geqrf\_compact}, and the per-matrix LAPACK loop;
Figure~\ref{fig:speedup-x86} recasts it as speedups over the per-matrix baseline.
Two effects stand out. First, the compact format's regime is the small sizes:
both compact paths beat the per-matrix loop by a wide margin at the smallest
orders (several-fold at $n\!\lesssim\!32$), the advantage shrinking with $n$ until,
around $n\!\approx\!128$, per-matrix LAPACK draws level and then overtakes --- its
internally blocked, cache-blocked \code{geqrf} scales into a regime the unblocked
compact kernel is not meant for. This is the expected crossover, and it places the
compact format exactly where the motivating applications live (stencils of a few
tens up to ${\sim}150$ points). Second, \cqr{} matches or beats MKL's own compact
\code{geqrf} across essentially the whole range, so the open kernel costs nothing
against the closed one on this machine. \draftnote{quote the small-$n$ speedup, the
crossover order, and the cqr/MKL geometric mean from the pinned-node run.}
...
\emph{Measured on a shared VM (provisional).}}
\label{fig:tput-x86}
\end{figure}
...
\caption{Speedup over the per-matrix LAPACK baseline on x86 (log scale; dashed
line is parity). The compact paths win at small orders and cross below parity
near $n\!\approx\!128$, where blocked per-matrix LAPACK takes over; \cqr{} stays
at or above MKL Compact throughout. \emph{Measured on a shared VM (provisional).}}
...
\paragraph{Absolute rate against the roofline.}
Throughput ratios say who wins; they do not say how much performance is left on
the table. Figures~\ref{fig:gflops-x86} and~\ref{fig:gflops-arm} therefore plot
the achieved rate (GFLOP/s) of all three drivers --- \cqr{}, the vendor compact
kernel, and the per-matrix LAPACK loop --- against the machine's
double-precision compute ceiling measured with LIKWID~\cite{likwid2010}. Three
things to read off them. First, the compact paths lead in absolute GFLOP/s at
the small orders but plateau early: the unblocked kernel is latency- and
memory-bound and never approaches the FMA peak. Second, per-matrix LAPACK does
the opposite --- low at small $n$, then climbing steeply as blocking and cache
reuse engage, overtaking the compact paths and running several times closer to
the ceiling at the largest orders; the throughput crossover is, in absolute
units, this gap opening. Third, the gentle staircase on the \cqr{} curve, dipping
at orders that are not multiples of the interleave width --- the remainder effect
of Section~\ref{sec:cqr}, and the reason the size list includes the odd stencil
sizes.

\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/gflops_x86.pdf}
\caption{Achieved rate (GFLOP/s) of \cqr{}, MKL Compact, and per-matrix LAPACK
on x86 (AVX-512), against the LIKWID-measured double-precision peak.
\emph{Rates measured on a shared VM (provisional); roofline from LIKWID.}}
\label{fig:gflops-x86}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/gflops_arm.pdf}
\caption{Achieved rate (GFLOP/s) of \cqr{}, ArmPL interleave-batch, and
per-matrix LAPACK on Arm (SVE), against the LIKWID-measured double-precision
peak. \emph{Placeholder data; placeholder roofline.}}
\label{fig:gflops-arm}
\end{figure}
...
the work, and there is no reason for it to remain closed or ISA-locked. A separate
cross-language check points the same way: a batched compact QR written in
idiomatic Fortran --- whole-array syntax or an explicit width-8 loop, no
intrinsics --- lands within ${\sim}10$--$40\%$ of these C++ kernels and itself
beats or ties MKL, evidence that the interleaved layout, not the language or the
intrinsic set, carries the performance.

The scope limits are deliberate and mirror the vendors'. \cqr{} is real-only;
complex Hermitian QR is future work. Its QR is unpivoted --- unlike ArmPL's
rank-revealing \code{?geqrfrr} --- so it does not reveal rank, and \cqr{} provides
neither LU nor GEMM (both deferred to the vendor libraries,
Section~\ref{sec:vendors}); the target least-squares and interpolation problems
are, however, set up to be well conditioned. It omits \code{larfg}'s
...
standard~\cite{bblas2021,bblas-standard}. Two broad strategies then extract the
cross-matrix parallelism. The \emph{interleaved/compact} layout --- the vendors'
answer for the small-matrix regime (Intel Compact~\cite{mkl-compact}, Arm
interleave-batch~\cite{armpl-interleave}) and this paper --- rewrites the data so
one SIMD instruction advances $V$ matrices. \emph{Kokkos
Kernels}~\cite{kokkos-kernels,kokkos-kernels-paper} reaches the same batched
parallelism by different means. Its kernels are C++ templates with the operation
properties (transpose, upper/lower, side, and the blocking \code{Algo}) as
compile-time arguments, so each is a header-only \code{invoke()} that inlines into
the user's \code{parallel\_for} and, through Kokkos, runs on CPUs and GPUs alike.
Crucially, it still vectorizes \emph{across} matrices when asked: making a
\code{View}'s scalar a SIMD vector type (\code{Vector<SIMD<T>,\,L>}) packs $L$
matrices into the lanes, and the same generic kernel advances all $L$ at once ---
the compact idea expressed through the type system rather than a storage format.
That composes with the \code{Serial}/\code{Team}/\code{TeamVector} execution levels
(Section~\ref{sec:vendors}) and, on GPUs, gives way to them. So the contrast with
the compact APIs is not \emph{whether} the batch reaches the SIMD units --- it
does --- but \emph{how} the interleaving is expressed: a SIMD element type reused
by generic kernels (Kokkos) versus a dedicated storage format and matching C
routines (MKL, ArmPL, \cqr{}). \cqr{} sits in the latter, format-based camp, as
does \code{batmat}~\cite{batmat} (aimed at convex optimization); its contribution
there is narrow and specific: an \emph{open} drop-in for the MKL compact QR,
portable across ISAs from one source, that supplies the apply-$Q$ step MKL omits. \draftnote{if targeting a venue, add the closest
