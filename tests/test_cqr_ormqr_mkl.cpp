 ...
 *   mkl_dgeqrf_compact -> cqr_mkl<T>::ormqr('T') -> mkl_dtrsm_compact and
...
int suite1(MKL_LAYOUT layout, char side, char trans, int nm, int m, int n, int k)
{
    ...
    MKL_INT info = 99;
    T wq;
    cqr_mkl<T>::ormqr(layout, side, trans, m, n, k, ap, ldap, taup, cp, ldcp, &wq, -1,
                      &info, fmt, nm);
    cqr_mkl<T>::ormqr(layout, side, trans, m, n, k, ap, ldap, taup, cp, ldcp, &wq,
...
template <class T> int suite2(int nm, int n, int nrhs)
{
    ...
    cqr_mkl<T>::ormqr(MKL_COL_MAJOR, 'L', 'T', m, nrhs, k, ap, m, taup, cp, m, &wq, -1,
                      &info, fmt, nm);
    cqr_mkl<T>::ormqr(MKL_COL_MAJOR, 'L', 'T', m, nrhs, k, ap, m, taup, cp, m, &wq,
                      (MKL_INT)wq, &info, fmt, nm);
