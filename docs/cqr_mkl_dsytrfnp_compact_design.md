# API Design Document: `cqr_mkl_dsytrfnp_compact` / `cqr_mkl_dsytrsnp_compact` / `cqr_mkl_dsysvnp_compact`
...
(interleaved-batch) format, the **solve** that completes a batched symmetric
linear system from that factor, and the **fused driver** that does both in one
pass over the batch. It follows the API style of `mkl_?potrf_compact`, but MKL
itself ships no compact `sytrf` of any kind, so -- like `cqr_mkl_?ormqr_compact`
-- these routines fill a gap in the compact ecosystem rather than shadowing an
MKL routine. The `np` (no-pivoting) suffix follows MKL's own naming for its
unpivoted compact LU,
[`mkl_?getrfnp_compact`](https://www.intel.com/content/www/us/en/docs/onemkl/developer-reference-c/2025-2/mkl-getrfnp-compact.html).
...
solve companion pairs with it the way LAPACK pairs `?sytrs` with `?sytrf`, and
the fused driver is the `?sysv` of the pair:
...
cqr_mkl_dsysvnp_compact ('L', A -> (L, D), B); // both, fused per group
```

All three are portable, open implementations built on the project's GNU-vector
...
compact `sytrf` to mirror); the solve is LAPACK `?sytrs` minus `ipiv`, plus the
three arguments MKL's compact routines add (`layout`, `format`, `nm`), with
`info` as a scalar status; the fused driver is LAPACK `?sysv` minus `ipiv` and
`work`/`lwork`, i.e. the solve's signature with a non-const `ap`. Both real
precisions are provided (`d`/`s`). None of the routines needs workspace, so
there are no `work`/`lwork` arguments.
...
Fused solve (`cqr_mkl_?sysvnp_compact`): the solve's arguments, with **`ap`**
(`double *`) the symmetric input batch, as for the factorization.
...
* **`ap`** (factorization, fused solve): the named triangle is overwritten with
  the factor -- `D` on the diagonal, the unit-diagonal `L`/`U` strictly off it,
  in Compact format; the other triangle is left untouched.
* **`bp`** (solve, fused solve): overwritten with the solution `X`, in Compact
  format.
* **`info`** (`MKL_INT *`): a single scalar status, `0` on success. No routine
  reports a zero pivot the way LAPACK reports `info = j` (section 6.2). The one
...
kernel (section 6.4); `?potrf` sets the same precedent with `A = U^T U`.
...
### 6.4 Layouts and triangles: one kernel over transposed views

As for `potrf`, the kernel factors the lower triangle of a `BatchView` and the
four `(layout, uplo)` combinations are that one kernel over different views:
the view of `A` for `MKL_LOWER`, the transposed view for `MKL_UPPER` (the
duality of section 6.3), each in the layout's strides. Two of the four are
contiguous along a factor column (unit row stride) -- column-major + `MKL_LOWER`
and its transpose dual row-major + `MKL_UPPER` -- and are the tuned cases; the
other two (column-major + `MKL_UPPER`, row-major + `MKL_LOWER`) walk the factor
across the strided axis and are correctness-first, not separately SIMD-tuned.
...
*unit* diagonal -- `?trsm` with `diag = 'U'` never reads the stored diagonal,
which is exactly what lets `D` live there -- so column-major runs on `trsm`'s
tuned side-left row-dot path and row-major on its strided kernel. Between them
sits the diagonal solve `B(i,:) *= 1/D(i)`, the one step `trsm` cannot express:
an `O(n*nrhs)` row scaling (one reciprocal per factor row, reused across the
`nrhs` columns), negligible next to the `O(n^2*nrhs)` sweeps and therefore
implemented once, over the views, for all layouts. No workspace is needed and
...
`cqr_mkl_?sysvnp_compact` is the `?sysv` of the pair, and it exists for
...
Because it calls the *same* group kernels in the *same* order on the *same*
data, its factor and its `X` are bit-identical to the two separate calls -- a
property the test suites gate (section 7.3). It also gives the library-side
threading of the whole solve that the separate calls only get per routine.
...
### 7.2 Suite 2 -- Cross-check vs `mkl_?getrfnp_compact`
...
factored by `cqr_mkl_?sytrfnp_compact` (col-major, lower) and by MKL's
`mkl_?getrfnp_compact`, and the strict lower triangles and diagonals are
compared elementwise at the cross-check tolerance (`1e-9` FP64, `1e-4` FP32) --
two independent implementations agreeing on the same pivots (observed agreement
is at the `1e-15` level in FP64).

### 7.3 Suite 3 -- End-to-end indefinite solve `A X = B`

`B = A X` for known `X` on indefinite batches, then `cqr_mkl_?sytrfnp_compact
-> cqr_mkl_?sytrsnp_compact` must recover `X`, over both `uplo`, both layouts,
padded partial groups, and RHS counts that exercise `trsm`'s 4/2/1 column
blocks. The system residual `A Xhat - B` is gated at `100 * n * eps` (what the
backward-stable sweeps control); the forward error `Xhat - X` additionally
...
* LAPACK-style `info = -j` argument validation of the three portable C APIs.

Both suites run in FP64 and FP32 and are CTest-registered.
...
`cqr::detail::pack<T,V>` machinery, the `BatchView` addressing, the
`for_each_group` threaded group loop, and (for the solve) the `trsm` group
kernels.

### 8.1 API boundary

* **MKL-style API** (`cqr_mkl_ext.h`, the primary surface):
  `cqr_mkl_?sytrfnp_compact` / `cqr_mkl_?sytrsnp_compact` /
  `cqr_mkl_?sysvnp_compact`, unwrapping `MKL_COMPACT_PACK -> V` and
  `MKL_UPLO`/`MKL_LAYOUT`, instantiated on `MKL_INT` so ILP64 dimensions are
  not narrowed.
* **Portable C API** (`cqr_compact.h`): `?sytrfnp_compact` / `?sytrsnp_compact`
  / `?sysvnp_compact`, taking `char layout`/`char uplo`, an explicit interleave
  width `V`, and no MKL dependency, with LAPACK-style `info = -j` argument
  validation.
* **Templated kernels**: `sytrfnp_compact_group<T,V>` factors the lower
  triangle of a `BatchView`; `sytrsnp_compact_group<T,V>` runs the three sweeps
  on one group; the all-groups drivers `sytrfnp_compact` / `sytrsnp_compact` /
  `sysvnp_compact` (the entry points both C adapters call) build the views for
  `layout`/`uplo` and loop over groups with `for_each_group`.

### 8.2 Source layout

| File | Role |
|------|------|
| `src/cqr_sytrfnp_compact.hpp` | Templated SIMD unpivoted-LDL^T group kernel and all-groups driver (scalar `T`, width `V`). |
| `src/cqr_sytrsnp_compact.hpp` | Templated solve: unit `trsm` group sweeps around the diagonal solve, and its driver. |
| `src/cqr_sysvnp_compact.hpp` | The fused factor-and-solve driver over both group kernels. |
| `src/cqr_compact.cpp` | Portable `?sytrfnp_compact` / `?sytrsnp_compact` / `?sysvnp_compact` C entry points (runtime `V` dispatch, `info = -j`). |
| `src/cqr_mkl_ext.cpp` | The MKL-style adapters (`MKL_COMPACT_PACK` -> `V`, `MKL_UPLO`/`MKL_LAYOUT` -> flags). |
| `tests/test_cqr_sytrfnp_compact.cpp` | Self-contained correctness test vs a scalar reference (no BLAS), FP64 and FP32. |
| `tests/test_cqr_sytrfnp_mkl.cpp` | MKL validation (invariants, `getrfnp` cross-check, indefinite solve, fused bit-identity), FP64 and FP32. |

The prototypes are added to `cqr_mkl_ext.h` (MKL-style) and `cqr_compact.h`
(portable); the test helpers `gen_sym_ldlt` and `ldlt_reconstruct` and the
`compact<T>` / `cqr_mkl<T>` / `mkl<T>` dispatch entries live in
`tests/test_compact_util.hpp` and `tests/test_mkl_util.hpp`.
