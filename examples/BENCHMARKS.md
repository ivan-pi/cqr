...
| [`bench_qr_compact`](#bench_qr_compact) | end-to-end QR *solve* `AX = B` | fully-open compact pipeline (three steps, and the one-call `gels`) vs MKL's pipeline vs per-matrix LAPACK |
...
(`X = R^-1 Q^T B`, single RHS), four ways:
...
the same pack and unpack, so gels-vs-cqr-batch is what the fusion buys, and
gels-vs-unbatched the headline batched win. Every path is checked against the
known solution `X == 1`, so the reported error is a forward error, not a
